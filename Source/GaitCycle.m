...
% Class for using OpenSim analysis data to perform calculations.
    ...
    methods
        ...
        function result = calculateMoS(...
                obj, cutoff, leg_length, direction, mode)
            
            % Analysis requirements.
            obj.require({'GRF', 'IK', 'BK'});
                
            % Compute extrapolated centre of mass.
            xcom = obj.calculateXCoM(leg_length);
            
            % Compute the BoS map.
            bos_polygons = obj.computeBoS(cutoff);
        ...
        function result = calculateXPMoS(obj, leg_length, direction, mode)
            ...
            % Compute the BoS map.
            bos_polygons = obj.computeXPBoS(leg_length);
        ...
        function visualise(obj, bos_params, frame, filename, varargin)
    ...
    methods (Access = private)
        ...
        function xcom = calculateXCoM(obj, leg_length)
            
            [p, v] = obj.getCoMPositionAndVelocity();
            
            xcom.x = extrapolatePendulum(p.x, v.x, leg_length);
            xcom.z = extrapolatePendulum(p.z, v.z, leg_length);
        ...
        function [points, polygons, lines, labels] = ...
            ...
            if any(strcmp('XPBoS', varargin))
                polygons = [polygons {obj.computeXPBoS(leg_length)}];
                labels = [labels 'XPBoS'];
            end
            ...
            if any(strcmp('MoS', varargin))
                ...
                u_max = calculateUMax(obj.computeBoS(cutoff), xcom.x, xcom.z);
            ...
            if any(strcmp('XPMoS', varargin))
                ...
                u_max = calculateUMax(...
                    obj.computeXPBoS(leg_length), xcom.x, xcom.z);
                lines = [lines {obj.constructLine(u_max.x, xcom)} ...
                    {obj.constructLine(u_max.z, xcom)}];
        ...
        % Get the indices corresponding to stance phase using GRF data.
            ...
            vert = 'vy';
            indices = ...
                find(obj.MotionData.GRF.Forces.getColumn([foot vert]) > cutoff);
        ...
        function marker_data = extrapolateMarkers(obj, leg_length)
        ...
        function polygons = computeXPBoS(obj, leg_length)
            ...
            markers = obj.extrapolateMarkers(leg_length);
        ...
        function polygons = computeBoS(obj, cutoff)
            ...
            lss_frames = obj.isolateStancePhase(lead, cutoff);
            oss_frames = obj.isolateOffStancePhase(off, cutoff);
